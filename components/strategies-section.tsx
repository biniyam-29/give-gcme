...
export default function StrategiesSection({ openDonationModal }: StrategiesSectionProps) {
  ...
  const calculateDimensions = useCallback(() => {
    ...
    const firstCard = strategiesContainerRef.current.querySelector('.group.flex-\\[0_0_400px\\]');
    if (firstCard) {
    ...
    const firstStrategyGroup = strategiesContainerRef.current.querySelector('.flex-nowrap.shrink-0');
  ...
  useEffect(() => {
    const startSlide = () => {
      if (isHovered || cardWidthWithMargin.current === 0 || totalStrategiesWidth.current === 0) {
      ...
      animationTimeoutRef.current = setTimeout(() => {
        ...
        if (Math.abs(currentTranslateX.current) >= totalStrategiesWidth.current) {
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-8 md:mb-12">
          ...
          <p className="text-lg md:text-xl text-neutral-600 max-w-2xl mx-auto">
            Explore the core strategies that drive our mission to make a lasting impact.
          </p>
        </div>
        ...
        >
          ...
          >
            {Array(3).fill(null).map((_, groupIndex) => (
              <div key={`strategy-group-${groupIndex}`} className="flex flex-nowrap shrink-0">
                  ...
                  >
                    <Link href={`/strategies/${strategy.slug}`}>
                      ...
                      <CardContent>
                        ...
                        <h4 className="font-medium text-neutral-700 mb-2">Key Activities:</h4>
                        <ul className="space-y-1 text-sm text-neutral-600">
                          {strategy.keyPoints.slice(0, 3).map((point, pointIndex) => (
                            <li key={pointIndex} className="flex items-center">
                              <ArrowRight className="w-3 h-3 mr-2 text-primary-500 flex-shrink-0" />
                              <span>{point}</span>
                            </li>
                          ))}
                          {strategy.keyPoints.length > 3 && (
                            <li className="text-sm text-primary-600 font-medium mt-2">
                              +{strategy.keyPoints.length - 3} more activities
                            </li>
                          )}
                        </ul>
                      </CardContent>
                    ...
                    <div className="p-6 pt-0">
                        ...
                        >
                            <Link href={`/strategies/${strategy.slug}`}>Learn More</Link>
                        </Button>
                    </div>
                  </Card>
                ))}
