...
import { MapPin, ArrowLeft, Clock, AlertTriangle, CheckCircle, TrendingUp } from "lucide-react"
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <main className="container mx-auto px-4 py-8">
        <Link href="/projects" className="inline-flex items-center text-stone-600 hover:text-stone-900 mb-6">
          ...
          <div className="lg:col-span-2 space-y-8">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-stone-600 mb-6">{project.shortDescription}</p>
                
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  <div className="text-center p-3 bg-white rounded border">
                    <div className="text-lg font-bold text-stone-800">{project.fundingRaised.toLocaleString()} ETB</div>
                    <div className="text-xs text-stone-600">Raised</div>
                  </div>
                  <div className="text-center p-3 bg-white rounded border">
                    <div className="text-lg font-bold text-stone-800">{project.fundingGoal.toLocaleString()} ETB</div>
                    <div className="text-xs text-stone-600">Goal</div>
                  </div>
                  <div className="text-center p-3 bg-white rounded border">
                    <div className="text-lg font-bold text-stone-800">{project.beneficiaries}</div>
                    <div className="text-xs text-stone-600">Beneficiaries</div>
                  </div>
                  <div className="text-center p-3 bg-white rounded border">
                    <div className="text-lg font-bold text-stone-800">{project.duration}</div>
                    <div className="text-xs text-stone-600">Duration</div>
                  </div>
                </div>

                <Progress value={(project.fundingRaised / project.fundingGoal) * 100} className="mb-6" />
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <p className="text-stone-600 whitespace-pre-line">{project.problem}</p>
                </div>
                <div>
                  <h3 className="font-semibold mb-2">Our Solution</h3>
                  <p className="text-stone-600 whitespace-pre-line">{project.solution}</p>
                </div>
                <div>
                  ...
                  {project.timeline.map((phase: any, index: number) => (
                    <div key={index} className="flex items-start gap-4">
                      <div className="flex-shrink-0">
                        ...
                        {phase.status === "in-progress" && <Clock className="w-6 h-6 text-blue-500" />}
                        {phase.status === "upcoming" && <AlertTriangle className="w-6 h-6 text-yellow-500" />}
                      </div>
                      <div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <ul className="list-disc list-inside text-stone-600 space-y-2">
                    {project.urgencyFactors.map((factor: string, index: number) => (
                ...
                <div className="space-y-4">
                  {project.testimonials.map((testimonial: any, index: number) => (
                    <div key={index} className="bg-stone-50 p-4 rounded-lg">
                      <p className="text-stone-600 italic mb-2">"{testimonial.quote}"</p>
